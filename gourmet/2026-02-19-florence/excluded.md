# 佛羅倫斯排除清單（Excluded Places）

本文件記錄被排除的候選餐廳/咖啡館/甜點店，以及排除理由。
根據 AGENTS.md 指引，排除決策必須有據可查，避免無聲刪除。
...
**Reason for Exclusion**: 觀光化嚴重，價格過高，品質不穩定
...
- 多次出現在「避開餐廳」討論串
- 本地人不推薦
- 建議選擇其他更真實的 trattoria

**Score (50-point rubric)**:
- Taste / Quality: 5/10（品質不穩定，部分評論稱平庸）
- Value: 3/10（價格過高，性價比極差）
- Convenience: 7/10（市中心位置，但這無法補償其他缺點）
- Consistency: 4/10（非常不穩定，評價兩極）
- Risk (10 = low risk): 7/10（高失望風險）
- **Total: 26/50** ❌

**Exclusion Decision**: 
評分 26/50 低於 30 分門檻，且多來源警告觀光陷阱。建議選擇其他更真實的 trattoria 如 Trattoria Sergio Gozzi, Osteria Vini e Vecchi Sapori, 或 L'Osteria di Giovanni。
...
**Initial sources**: TripAdvisor, guides  

**Why not researched further**:
- 初步評論顯示價格偏高
- 與更具特色的選項（如 La Buchetta）相比優勢不明顯
- 時間有限，優先研究更獨特選項
...
所有排除決策基於：
- **多來源交叉驗證**（Yelp, TripAdvisor, Google Maps, Reddit, 專業美食指南）
- **50點評分系統**（Taste/Quality, Value, Convenience, Consistency, Risk）
- **明確排除門檻**（總分 < 30/50 或 hard exclusion triggers）
- **文檔化理由**（所有排除都有據可查）

不排除未來重新評估或新資訊出現後調整決策。
